...
$$
\mathrm{^{\small\ A}E}\ \ \mathrm{or}\ \mathrm{^{\small\ A}_{\small\ Z}E}
$$

For example, carbon 12 (atomic number 6) and uranium 235 (atomic number 92) are typeset with and without atomic numbers as shown below. The latter form, without *Z*, is the default in formatdown. 

```{r}
#| echo: false

hyphenated <- c(
  "$\\small\\mathrm{C}$--$\\small\\mathrm{12}$",
  "$\\small\\mathrm{U}$--$\\small\\mathrm{235}$"
)
x <- c("C-12", "U-235")
...
where the `\mathrm{}` macro creates serif, non-italic text, the carat (`^{}`) creates the superscript for the mass number, the `\small` macro reduces the font size of the superscript, and the space macro (`\_ `) separates the small macro from the mass number. Rendering this expression in an Rmd script as a *LaTeX-style inline equation* yields: $\mathrm{^{\small\ 12}C}$.

To *program* the markup, however, we enclose it in quote marks as a character string, that is, 

        "$\\mathrm{^{\\small\\ 12}C}$", 

which requires us to "escape" the backslash in `\mathrm`, `\small`, etc.,  by adding extra backslashes. Rendering this expression in an Rmd script as an *inline R code chunk* yields the same result: `r "$\\mathrm{^{\\small\\ 12}C}$"`. 
...
*Examples.* &nbsp; Scalar and vector examples are usually rendered using inline code chunks in an Rmd script. 
...
Example 1 renders as:  `r format_nucl(x)`. 
...
Example 2 renders as: `r format_nucl(y)`
...
Example 3 renders as: `r format_nucl(x, Z = TRUE)`
...
Example 4 renders as: `r format_nucl(y, Z = TRUE)`
...
Example 5 renders as: `r format_nucl(x)`
...
Example 6 renders as: `r format_nucl(y)`
...
*Examples.* &nbsp; In this example, the first entry has an incorrect element symbol and the second has an incorrect mass number.  
...
*Examples.* &nbsp; If we use the atomic number argument, the letter *Z* is placed in the subscript. 
...
*Examples.* &nbsp; As noted earlier, the usual `Z` argument in this case places a *Z* in the subscript position. 
...
*Examples.* &nbsp; Some authors prefer using *X* for the general element symbol, 
