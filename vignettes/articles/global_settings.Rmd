...
<div>
<img src="options-img.png" alt="An image of the word 'option'." width="80%"><br>*Option* by Mike Lawrence is licensed under CC by 2.0 DEED.
</div>
...
Sometimes the default `$ ... $` delimiters fail to render correctly. I encountered this once using `kableExtra::kbl()` in a `.qmd` document. The solution, suggested by [@Mathjax:2024], is to use the delimiter pair `\( ... \)`, hence the built-in alternate, `delim = "\\("`. 
...
Logical, length 1, default FALSE, used to determine whether or not the atomic number subscript is used in nuclear notation. For example, carbon-12 can be represented as `r format_nucl("C-12")` or, with Z = TRUE, `r format_nucl("C-12", Z = TRUE)`. 

warn:

Logical, length 1, default TRUE, used to determine whether or not a warning is issued when an input for a chemical element in hyphenated notation contains an error in the element symbol or the mass number of the isotope. Set to FALSE when using nuclear notation with general symbols, e.g., `r format_nucl("E-A", Z = TRUE, warn = FALSE)`. 
