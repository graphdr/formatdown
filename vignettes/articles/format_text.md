...
<img src="latex_math_formatting_text_table_2.png" alt="Table of text formatting macros in formatdown" width="60%">

</div>

Columns of text in a data table will generally not be rendered in the
same typeface as produced by `format_numbers()`. The `format_text()`
...
Example 3 renders as:   $\small \mathrm{abc}$, $\small \mathrm{def}$,
$\small \mathrm{NA}$.
...
Example 4 renders as:   $\small \mathrm{10}$, $\small \mathrm{3e-05}$,
$\small \mathrm{4.56e+10}$.
...
Example 7 renders as:   $\small \mathrm{2026-05-01}$.
...
Example 8 renders as:   $\small \mathrm{low}$, $\small \mathrm{med}$,
$\small \mathrm{high}$.
...
The argument of `format_text()` is evaluated within a math-markup. Thus
any math syntax in your text, such as an underscore “\_” or caret “^”,
are rendered in math mode, not verbatim. For example, the underscore
creates a subscript and the caret creates a superscript,
