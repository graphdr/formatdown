...
1. `r format_dcml(x, delim = "\\(")`

1. `r format_text(txt)`

1. `r format_text(txt, delim = "\\(")`
...
10. `r format_dcml(x, 5, decimal_mark = ",")`

10. `r format_sci(y, 5, decimal_mark = ",")`
...
The interval arguments `big_interval` and `small_interval` set the number of digits separated by thin spaces when `big_mark` or `small_mark` are not empty. However, formatdown does not encode the exemption for 4-digit groups mentioned in the IST quote above.  
...
12. `r format_dcml(w, big_mark = "thin")`

12. `r format_dcml(x, big_mark = "\\\\,")`

12. `r format_dcml(y, small_mark = "\\\\,")`

12. `r format_dcml(y, small_mark = "\\\\,", small_interval = 3)`

12. `r format_dcml(z, 12, big_mark = "thin", small_mark = "\\\\,")`



## Preserving text spaces

The horizontal-space macro is used to preserve spaces in text formatted with `format_text()`. Without it, an inline math markup such as 
...
Because the backslashes must be escaped, the formatdown output is $\small\mathtt{\verb|"\\>"|}$, but the the argument value set by the user is  `word_space =` $\small\mathtt{\verb|"\\\\>"|}$. One may also use $\mathtt{\small\verb|"\\\\:"|}$ or $\mathtt{\small\verb|"\\\\ "|}$. 
...
18. `r format_text(txt)` 

18. `r format_text(txt, word_space = "\\\\:")`
