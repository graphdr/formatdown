...
Using `air`, a data frame included with formatdown that contains columns of class Date, character, factor, numeric, and integer.
...
# Render in document
knitr::kable(air, align = "r")
```

Treating a column as a vector, 

```{r}
# Copy to avoid "by reference" changes to air
DT <- copy(air)

# Format as a vector
format_power(DT$pressure, digits = 4)
```

is rendered as `r format_power(DT$pressure, digits = 4)`. 
...
DT$pressure <- format_power(DT$pressure, digits = 4)
DT[]
...
cols_we_want <- c("temperature", "pressure", "density")
...
cols_we_want <- c("temperature", "density")
...
DT$p_Pa <- format_power(DT$pressure, 4)
DT[]
