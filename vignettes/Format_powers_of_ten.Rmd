...
DT[, alt   := round(alt/1000, 0)]
DT[, temp  := round(temp, 1)]
DT[, pres  := round(pres, 0)]
DT[, sound := round(sound, 1)]

# Apply power of ten notation 
DT[, dens  := format_power(dens, 3, format = "sci", set_power = -3, omit_power = c(-2, 2))]
...
# Round to appropriate decimal and add inline math markup
DT[, alt   := format_decimal(alt/1000, 0)]
DT[, temp  := format_decimal(temp, 1)]
DT[, pres  := format_decimal(pres, 0, big_mark = ",")]
DT[, sound := format_decimal(sound, 1)]
