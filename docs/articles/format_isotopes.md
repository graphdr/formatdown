...
[`format_nucl()`](https://graphdr.github.io/formatdown/reference/format_nucl.md)
obtains atomic numbers from the built-in `element_set` data by matching
...
**Notating a general form.**   Because
[`format_nucl()`](https://graphdr.github.io/formatdown/reference/format_nucl.md)
will format any character string with a single hyphen between two
character strings, we can use it to typeset the isotope general form
such as \mathrm{^{A}E}. In such a case, we turn off the warning using
the `warn` argument.
